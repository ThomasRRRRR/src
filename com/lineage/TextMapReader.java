...
public class TextMapReader extends MapReader {
	...
	public Map<Integer, L1Map> read() {
		...
		for (final Integer key : mapDatas.keySet()) {
			...
			L1V1Map map = null;
			try {
				map = new L1V1Map(mapId, this.read(mapId, xSize, ySize),
						mapData.startX,
						mapData.startY,
    ...
    public L1Map read(final int id) throws IOException {
        ...
        for (final Integer key : mapDatas.keySet()) {
            ...
            L1V1Map map = null;
            if (mapId == id) {
                map = new L1V1Map(mapId, this.read(mapId, xSize, ySize), mapData.startX, mapData.startY, mapData.isUnderwater, mapData.markable, mapData.teleportable, mapData.escapable, mapData.isUseResurrection, mapData.isUsePainwand, mapData.isEnabledDeathPenalty, mapData.isTakePets, mapData.isRecallPets, mapData.isUsableItem, mapData.isUsableSkill, mapData.isGuaji, mapData.isClanPc, mapData.isPartyPc, mapData.isdropitem, mapData.isUsableShop, mapData.isStart_time, mapData.isEnd_time, mapData.isweek);
                return map;
            }
        }
