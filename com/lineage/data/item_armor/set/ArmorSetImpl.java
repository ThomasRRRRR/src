...
public class ArmorSetImpl extends ArmorSet {
	 ...
	 * @param ids 套裝物品編號陣列
	 */
	protected ArmorSetImpl(final int id, final int ids[], final int addEffectId) {
		this._id = id;
		this._ids = ids;
		this._addEffectId = addEffectId;
	...
	public int[] get_mode() {
		...
		for (final ArmorSetEffect effect : _effects) {
            ...
            if (effect instanceof Effect_getweaponskilldmg) {
	...
	public void giveEffect(final L1PcInstance pc) {
		try {
			...
			// 套裝效果動畫
            if (this._addEffectId != -1) {
                final L1SkinInstance skin = L1SpawnUtil.spawnSkin(pc, this._addEffectId);
                if (skin != null) {
                    skin.setMoveType(1);
                    pc.addSkin(skin, this._addEffectId);
                }
            }
