	...
	public void tradeOK(final L1PcInstance pc) {
		...
		try {
			...
			if (trading_partner != null) {
				...
				// 取回對象交易物品
				final List<L1TradeItem> map_2 = trading_partner
						.get_trade_items();
				...
				if (!map_1.isEmpty()) {
					for (final Iterator<L1TradeItem> iter = map_1.iterator(); iter
						...
						final L1ItemInstance tg_item = pc.getInventory()
								.getItem(tg.get_objid());
						if (tg_item == null) {
							_log.error("人物交易異常(物品為空): " + pc.getName() + "/"
									+ pc.getNetConnection().hashCode());
							// pc.getNetConnection().kick();
							// trading_partner.getNetConnection().kick();
							continue;
						...
						// 移除物品
						pc.getInventory().removeItem(tg.get_objid(),
								tg.get_count());

						// 創出物品
						if (tg_item.isStackable()) {
							final L1ItemInstance tgItem = ItemTable.get()
									.createItem(tg_item.getItemId());
...
//								"交易記錄",
//								tg_item.getLogName() + "("
//										+ tg_item.getItemId() + ")物件objid為"
//										+ tg_item.getId() + "交易對象ID"
//										+ trading_partner.getId() + "交易對象名字"
//										+ trading_partner.getName() + "玩家ID"
//										+ pc.getId() + "玩家名字" + pc.getName());
						
						// 個人交易物品紀錄
//						RecordTable.get().recordeTrade(pc.getName(), trading_partner.getName(), tg_item.getAllName(), (int)tg.get_count(), tg_item.getId(), pc.getIp(), trading_partner.getIp());
					}
				}

				if (!map_2.isEmpty()) {
					for (final Iterator<L1TradeItem> iter = map_2.iterator(); iter
							.hasNext();) {
						final L1TradeItem tg = iter.next();
						if (!CharItemsReading.get().getUserItems(
								trading_partner.getId(), tg.get_objid(),
								tg.get_count())) {
							_log.error("人物交易異常(指定數據數量有誤): "
									+ trading_partner.getName()
									+ "/"
									+ trading_partner.getNetConnection()
											.hashCode());
							// pc.getNetConnection().kick();
							// trading_partner.getNetConnection().kick();
							continue;
						...
						final L1ItemInstance tg_item = trading_partner
								.getInventory().getItem(tg.get_objid());
						if (tg_item == null) {
							_log.error("人物交易異常(物品為空): "
									+ trading_partner.getName()
									+ "/"
									+ trading_partner.getNetConnection()
											.hashCode());
						...
						// 移除物品
						trading_partner.getInventory().removeItem(
								tg.get_objid(), tg.get_count());
...
//						WriteLogTxt.Recording("交易記錄", tg_item.getLogName()
//								+ "(" + tg_item.getItemId() + ")物件objid為"
//								+ tg_item.getId() + "玩家ID" + pc.getId()
//								+ "玩家名字" + pc.getName() + "交易對象ID"
//								+ trading_partner.getId() + "交易對象名字"
//								+ trading_partner.getName());
						// 個人交易物品紀錄
						
//						RecordTable.get().recordeTrade(trading_partner.getName(), pc.getName(), tg_item.getAllName(), (int)tg.get_count(), tg_item.getId(), trading_partner.getIp(), pc.getIp());
					}
				}
				...
				if (!temp2.isEmpty()) {// trading_partner
					for (Integer key : temp2.keySet()) {
						...
						if (count == tg_item.getCount()) {
							...
							trading_partner.sendPackets(new S_ItemStatus(
									tg_item, tg_item.getCount()));// 還原物品數量
						}
					}
				}
