...
/**
 * 龍印魔石(弓手) 體力上限+25、魔力上限+15、遠距離命中率+1
 * @author dexc
 *
 */
public class DS_AX05 extends SkillMode {

	private static final int _addhp = 25;
	private static final int _addmp = 15;
	...
	public int start(final L1PcInstance srcpc, final L1Character cha, final L1Magic magic, final int integer) throws Exception {
		final int dmg = 0;
		if (!srcpc.hasSkillEffect(L1SkillId.DS_AX05)) {
			srcpc.addMaxHp(_addhp);
			srcpc.addMaxMp(_addmp);
			srcpc.addBowHitup(_addhitbow);
			srcpc.setSkillEffect(L1SkillId.DS_AX05, integer * 1000);
