...
//		PerformanceTimer timer = new PerformanceTimer();
		...
		try {
			...
			pstm = con.prepareStatement("SELECT * FROM Manly_紋樣記錄");
		...
		//_log.info("讀取->[Manly_紋樣記錄]資料數量: " + _wenyangj.size() + "(" + timer.get() + "ms)");
	}
	...
	public void updateItem(L1PcInstance pc , final L1WenYangJiLu wenyang) {
		...
		try {
			...
			pm = co.prepareStatement(
					"UPDATE `Manly_紋樣記錄` SET `類型`=?,`強化等級`=? WHERE `玩家ID`=?");
	...
	public void storeItem(L1PcInstance pc , final L1WenYangJiLu wenyang) {
		...
		try {
			...
			co = DatabaseFactory.get().getConnection();
			pm = co.prepareStatement(
					"INSERT INTO `Manly_紋樣記錄` SET `玩家ID`=?,`類型`=?,`強化等級`=?");
			pm.setInt(++i, pc.getId());	
			pm.setInt(++i, wenyang.getType());
			pm.setInt(++i, wenyang.getLevel());		
			pm.execute();
