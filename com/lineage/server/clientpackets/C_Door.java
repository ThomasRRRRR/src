...
public class C_Door extends ClientBasePacket {
		...
		try {
			...
			if (obj instanceof L1DoorInstance) {
				...
				case 6007:
					...
					if (pc.getInventory().consumeItem(40313, 1)) { //巴風特監獄的鑰匙
						door.open();
						final CloseTimer closetimer = new CloseTimer(door);
						closetimer.begin();
					}
	...
	private boolean isExistKeeper(final L1PcInstance pc, final int keeperId) {
		if (keeperId == 0) {//管理員ID為0
			return false;
		}
		final L1Clan clan = WorldClan.get().getClan(pc.getClanname());
		if (clan != null) {
			final int houseId = clan.getHouseId();//自己血盟的盟屋ID
			if (houseId != 0) {
				final L1House house = HouseReading.get().getHouseTable(houseId);
				if (keeperId == house.getKeeperId()) {//管理員ID=自己血盟的盟屋管理員ID
					return false;//視為沒有管理員
				}
			}
		}
		return true;
