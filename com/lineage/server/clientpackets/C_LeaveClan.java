		...
		try {
			...
			if (clan != null) {
				...
				if (player.isCrown() && (player.getId() == clan.getLeaderId())) { // 盟主
					...
					// 同盟系統 by Manly
					if (ClanAllianceReading.get().getAlliance(
							player.getClanid()) != null) {
						// 同盟時無法解散血盟。
					...
					final L1ClanMatching Clan = L1ClanMatching.getInstance();
					...
					if (ConfigClan.clandelt) {
						player.setPcContribution(0);
                    }
					player.setClanContribution(0);
					player.setClanNameContribution("");
					player.setTitle("");
