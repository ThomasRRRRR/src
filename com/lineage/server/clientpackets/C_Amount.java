...
public class C_Amount extends ClientBasePacket {
	...
	public void start(final byte[] decrypt, final ClientExecutor client) throws Exception {
		try {
			...
			final L1Object object = World.get().findObject(objectId);
			L1NpcInstance npc = null;
			if (object instanceof L1NpcInstance){
				npc = (L1NpcInstance)object;
			}
			if (npc == null) {
				pc.getAction().action(s,amount);
				return;
			}
					...
					if (inn != null) {
						...
						if (item != null) {
							...
							pc.sendPackets(new S_ServerMessage(143, npc
									.getName(), itemName)); // \f1%0%s 給你 %1%o 。
