...
/**
 * 
 * 類名稱：CustomScheThreadPoolExecutor<br>
 * 類描述：取自JDK的ScheduledThreadPoolExecutor<br>
 * 創建人:Agui512<br>
 * 修改時間：2016年7月14日 下午12:01:41<br>
 * 修改人:QQ:1479366584<br>
 * 修改備註:<br>
 * @version<br>
 */
public class CustomScheThreadPoolExecutor extends CustomThreadPoolExecutor implements ScheduledExecutorService {
	...
	private class ScheduledFutureTask<V> extends FutureTask<V> implements RunnableScheduledFuture<V> {
		...
		ScheduledFutureTask(Callable<V> callable, long ns) {
			...
			this.period = 0;
			this.sequenceNumber = sequencer.getAndIncrement();
			this.runnableName = callable.toString();
		}
		...
		private void runPeriodic() {
			...
			if (ok && (!down || (getContinueExistingPeriodicTasksAfterShutdownPolicy() && !isTerminating()))) {
				...
				if (p > 0)
					...
					time = now() - p;
				CustomScheThreadPoolExecutor.super.getQueue().add(this);
			}
	 ...
	 */
	public CustomScheThreadPoolExecutor(int corePoolSize) {
	 ...
	 */
	public CustomScheThreadPoolExecutor(int corePoolSize, ThreadFactory threadFactory) {
	 ...
	 *             if handler is null
	 */
	public CustomScheThreadPoolExecutor(int corePoolSize, CustomRejectedExecutionHandler handler) {
	 ...
	 */
	public CustomScheThreadPoolExecutor(int corePoolSize, ThreadFactory threadFactory, CustomRejectedExecutionHandler handler) {
	 ...
	 *             cannot be accepted for execution because the executor has
	 *             been shut down.
	 * @throws NullPointerException
	 *             if command is null
	 */
	 ...
	 *         submitted using <tt>execute</tt>, which are for scheduling
