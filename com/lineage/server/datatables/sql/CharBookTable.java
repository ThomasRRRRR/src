...
public class CharBookTable implements CharBookStorage {
	...
	public void load() {
		...
		_log.info("讀取->記憶座標紀錄資料數量: " + _bookmarkMap.size() + "(" + timer.get() + "ms)");
	}
	...
	@Override
	public void updateBookmarkName(final L1BookMark bookmark) {
		Connection con = null;
		PreparedStatement pstm = null;
		try {
			con = DatabaseFactory.get().getConnection();
			pstm = con.prepareStatement(
					"UPDATE `character_teleport` SET `name`=? WHERE `id`=?");
