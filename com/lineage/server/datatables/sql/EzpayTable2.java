...
/*     */ 
/*     */ 
/*     */ public class EzpayTable2
/*     */   implements EzpayStorage2
/*     */ {
/*  23 */   private static final Log _log = LogFactory.getLog(EzpayTable2.class);
/*     */ 
/*     */ 
/*     */ 
/*     */ 
...
/*  39 */       String sqlstr = "SELECT * FROM `withdraws` WHERE `account`=? ORDER BY `id`";
...
/*  84 */       String sqlstr = "SELECT * FROM `withdraws` WHERE `account`=? AND `id`=?";
...
/* 129 */       String sqlstr = "UPDATE `withdraws` SET `isget`=1,`player`=?,`time`=?,`ip`=? WHERE `id`=? AND `account`=?";
...
/* Location:              D:\Desk\381server.jar!\com\lineage\server\datatables\sql\EzpayTable.class
